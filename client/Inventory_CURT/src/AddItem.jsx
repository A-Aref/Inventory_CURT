...
function AddItem(props) {
  ...
  fetch("/api/addItem", {
    ...
    body: JSON.stringify({ name: name, quantity: quantity, description: description, category: props.category}),
  })
    .then((response) => {
        if (response.ok) {
    ...
    <>
      <Modal show={props.show} onHide={handleClose} data-bs-theme="dark">
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={handleAddItem} disabled={!enableAdd}>
            Add item
          </Button>
        </Modal.Footer>
      </Modal>
